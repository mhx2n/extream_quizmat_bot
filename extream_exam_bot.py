...
# === Emoji Quiz Feature Start ===

# ✅ Emoji Quiz Feature for Ultron Bot (Modular Extension)
# This module integrates with your existing Ultron bot.
# It enables emoji-based anonymous quizzes with explanations, user stats, and controlled access.
...
async def send_emoji_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    if not user or user.id != OWNER_ID:
        await update.message.reply_text("Only the owner can post emoji quizzes.")
        return
    ...
    try:
        ...
        options = raw[2:6]
        explanation = "\n".join(raw[6:]).replace("ব্যাখ্যা:", "").strip()
        correct_letter = [i for i, line in enumerate(options) if line.startswith("*")]
        if not correct_letter:
            await update.message.reply_text("❌ সঠিক উত্তর * দিয়ে চিহ্নিত করতে হবে।")
            return

        correct_index = correct_letter[0]
        options = [line.replace("*", "").strip() for line in options]
        quiz_id = uuid.uuid4().hex[:8]

        kb = [
            [InlineKeyboardButton(EMOJI_OPTIONS[i], callback_data=f"eq:{quiz_id}:{i}") for i in range(2)],
            [InlineKeyboardButton(EMOJI_OPTIONS[i], callback_data=f"eq:{quiz_id}:{i}") for i in range(2, 4)]
        ]

        full_q = f"❓ <b>{question}</b>\n" + "\n".join(f"{EMOJI_OPTIONS[i]} {opt}" for i, opt in enumerate(options))
        full_q += f"\n\n🧠 কুইজ ID: <code>{quiz_id}</code>"

        context.bot_data[f"quiz_{quiz_id}"] = {
            "question": question,
            "options": options,
        ...
        await update.message.reply_text(full_q, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(kb))
    except Exception as e:
        await update.message.reply_text(f"❌ কুইজ তৈরিতে সমস্যা হয়েছে: {e}")
...
async def handle_emoji_response(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    data = q.data.replace("eq:", "").split(":")
    if len(data) != 2:
        return
    quiz_id, selected = data[0], int(data[1])

    key = f"quiz_{quiz_id}"
    quiz = context.bot_data.get(key)
    if not quiz:
        await q.answer("❌ কুইজ পাওয়া যায়নি।", show_alert=True)
        return

    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT * FROM emoji_quiz_responses WHERE quiz_id=? AND user_id=?", (quiz_id, user_id))
    if cur.fetchone():
        await q.answer("🛑 আপনি আগেই উত্তর দিয়েছেন।", show_alert=True)
        return

    correct = int(quiz['correct'])
    is_right = int(selected == correct)
    expl = quiz['explanation']

    cur.execute("""
        INSERT INTO emoji_quiz_responses (quiz_id, user_id, selected_option, is_correct, explanation, clicked_at)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (quiz_id, user_id, selected, is_right, expl, datetime.datetime.utcnow().isoformat()))
    conn.commit()
    conn.close()

    if is_right:
        await q.edit_message_text(f"🎉 <b>সঠিক উত্তর!</b>\n\n{expl}", parse_mode=ParseMode.HTML)
    else:
        correct_emoji = EMOJI_OPTIONS[correct]
        await q.edit_message_text(f"❌ ভুল উত্তর।\n✅ সঠিক উত্তর ছিল {correct_emoji}\n\n{expl}", parse_mode=ParseMode.HTML)

async def my_quiz_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user_id = update.effective_user.id
    conn = db_connect()
    cur = conn.cursor()
    cur.execute("SELECT quiz_id, selected_option, is_correct, clicked_at FROM emoji_quiz_responses WHERE user_id=? ORDER BY clicked_at DESC LIMIT 5", (user_id,))
    rows = cur.fetchall()
    conn.close()

    if not rows:
        await update.message.reply_text("❌ আপনি এখনও কোন ইমোজি কুইজে অংশ নেননি।")
        return

    lines = ["📊 আপনার সর্বশেষ কুইজ স্ট্যাটস:"]
    for row in rows:
        status = "✅ সঠিক" if row["is_correct"] else "❌ ভুল"
        emoji = EMOJI_OPTIONS[row["selected_option"]]
        lines.append(f"🧠 <code>{row['quiz_id']}</code>: {emoji} → {status}")

    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

def register_emoji_quiz_handlers(app):
    emoji_quiz_init()
    app.add_handler(CommandHandler("emojiquiz", send_emoji_quiz))
    app.add_handler(CommandHandler("myquizstats", my_quiz_stats))
    app.add_handler(CallbackQueryHandler(handle_emoji_response, pattern=r"^eq:"))
